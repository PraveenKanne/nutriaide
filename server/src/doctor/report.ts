...
export async function generateReport(userId: string, from: Date, to: Date): Promise<Buffer> {
  ...
  const vitals = await prisma.vital.findMany({
    where: { userId, measuredAt: { gte: from, lte: to } }
  });
  ...
  meals.forEach(m => {
    doc.fontSize(10).text(`${m.timestamp.toISOString()} — ${m.mealType} — ${m.food.name}`);
  });

  doc.moveDown().fontSize(14).text('Medications');
  meds.forEach(m => {
    doc.fontSize(10).text(`${m.name} ${m.dose}${m.unit} ${m.route}`);
  });

  doc.moveDown().fontSize(14).text('Vitals');
  vitals.forEach(v => {
    doc.fontSize(10).text(`${v.measuredAt.toISOString()} — ${v.type}: ${v.value} ${v.unit}`);
  });
